...
bin\phpsdk_setshell.bat %CRT% %ARCH% && bin\phpsdk_setvars.bat && bin\phpsdk_dumpenv.bat && bin\phpsdk_buildtree.bat phpdev && cd php-%PHP_VER% && ..\..\..\..\bin\phpsdk_deps -u --no-backup && if NOT EXIST "..\deps\include\%DEPS%.h" (
	cd .. && ..\..\..\..\cmd\%DEPS%_build.bat && cd php-%PHP_VER% && buildconf --force --add-modules-dir=..\pecl\ && configure --enable-cli --with-ffi --with-iconv --enable-phar --enable-filter --with-openssl --enable-sockets --enable-mbstring --with-libxml --enable-fileinfo --enable-xmlwriter --enable-tokenizer --enable-embed %EXT% %ZTS% %OPTIONS% && nmake %SNAP% && cd ..\..\..\..\..
) else if EXIST config.nice.bat (
  config.nice.bat && nmake %SNAP% && cd ..\..\..\..\..
) else (
  buildconf --force --add-modules-dir=..\pecl\ && configure --enable-cli --with-ffi --with-iconv --enable-phar --enable-filter --with-openssl --enable-sockets --enable-mbstring --with-libxml --enable-fileinfo --enable-xmlwriter --enable-tokenizer --enable-embed %EXT% %ZTS% %OPTIONS% && nmake %SNAP% && cd ..\..\..\..\..
)
